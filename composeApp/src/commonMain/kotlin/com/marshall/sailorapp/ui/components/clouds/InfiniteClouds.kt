package com.marshall.sailorapp.ui.components.clouds
...
import com.marshall.sailorapp.data.model.Cloud
...
//            clouds.add(Cloud(x = 0f, y = y, size = size, speed = speed, alpha = alpha, color = color))
            ...
            repeat(Random.nextInt(3, 6)) { // Generate 3 to 5 normal clouds initially
                ...
                clouds.add(Cloud(x = Random.nextFloat() * screenWidth, y = y, size = size, speed = speed, alpha = alpha, color = color, isRainCloud = showRain)) // Position them anywhere on screen
            }
        }
    }
        ...
        while (true) {
            withFrameNanos { frameTime ->
                ...
                if (showClouds && !showRain && Random.nextFloat() < 0.02f) { // Only add new clouds if showClouds is true AND not raining
                    ...
                    clouds.add(Cloud(x = screenWidth + size * density * 2, y = y, size = size, speed = speed, alpha = alpha, color = color, isRainCloud = showRain)) // Start from right, adjusted for density
                }
            }
        }
    }
